...
game.updatePillars = function(){
  ...
  if( game.logics.lastPillar <= 0 ){
    ...
    do {
      ...
      var x = game.logics.halfw + ( Math.random() * ( game.logics.halfw ) * game.logics.lastPillarSide ) |0,
          width = ( game.logics.basePillarWidth + game.logics.addedPillarWidth * Math.random() / ( num + 1 ) ) |0,
          found = -1;

      for( var i = 0; i < game.logics.pillars.length; ++i )
        if( game.logics.pillars[ i ].dead )
          found = i;
